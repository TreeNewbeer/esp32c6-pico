...
提交前保留编辑器中的最新调整：背面 `LOGO_KZL` 整组平移 (+0.012374, −0.858847) mm，画布中心更新为 (109.012374, 111.091153) mm；尺寸、线宽与背面镜像保持不变。正面 logo 位置不变，当前坐标已同步至 `layout.json`。

2026-09-10：板宽由 20 mm 缩至 18 mm，仅左板边向右内收 2 mm；两个 logo 的位置、尺寸、图形和图层保持不变。板框尺寸已同步至 `layout.json`。

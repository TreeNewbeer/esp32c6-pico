...
- 默认用中文回复，称呼用户为“KZL 大人”；代码注释用英文。
- 本机为 Debian 13、zsh、KiCad Flatpak。截图位于 `~/Pictures/agents/`，按任务查看最新或指定截图。
- 在用户授权范围内直接完成修改和验证；保留已有改动，不回滚无关内容。改 PCB 前在 `tmp/` 中备份当前工程，避免编辑器中的旧文件覆盖磁盘更新。
- 项目新版本使用 `X.Y.Z`，跳过含数字 `4` 的版本值；日常小改不单独升版。第三方版本、型号和历史记录保持原样。
...
- 射频匹配 R/C（C2、C3、C16、C17、C18、R6）保持英制 0201；其余全部 38 个电阻、电容统一英制 0402（公制 1005），包括 C8、C19、C20、C22、C23 和 R17、R1–R5。保持容量、阻值、网络和 DNP 状态；大容量电源电容最低 10V，核对实际偏压下的有效容量，维护 [POWER_PASSIVES.md](POWER_PASSIVES.md) 与 BOM。电感和其他器件不随该阻容约定替换。
...
- 普通过孔孔边到两面 SMD 阻焊开窗至少 0.10 mm，包括同网络及 DNP 焊盘；普通孔双面盖油。仅保留 U1 的 9 个、U3 的 2 个中心散热焊盘内孔，必须填孔盖铜。

...
**F1 已按用户要求删除**：原理图和 PCB 均移除 1206 自恢复保险丝，USB 的 `VBUS_USB` 直接连接 D5 阳极，取消 `VBUS_FUSED` 网络。移除绕到板尾 F1 的走线，保留 D5 防反灌；其余 64 个器件的位置、安装面和封装不变。BOM、装配坐标及预览已同步，默认装配由 59 个减为 58 个。删除后的 ERC、DRC、未连接和原理图一致性均为 0；QA 仍为 45/51，六项既有失败不变。以下各轮检查数字为历史记录，以本段及当前报告为准。
...
| `output/package-changes.json` | 当前封装替换范围、尺寸、网表与检查基线 |

...
2026-09-09 排针内移后使用 KiCad 10.0.6 重新填铜并检查：**DRC 0 项、未连接 0 项、原理图一致性 0 项；独立 QA 44/49 通过，无新增失败项**。本轮未修改原理图，既有 ERC 报告为 0 项。当前报告为 `output/drc-final.json`、`output/qa.json` 和 `output/headers-inward.json`；`output/layout-20x29mm.json` 及此前两项孔距违规的记录属于历史轮次。
...
| `output/headers-inward.json` | 当前排针内移的尺寸、修改范围及检查基线 |
...
| `output/via-in-pad.csv` | 当前散热焊盘内孔清单，尚未满足 11 孔目标；须填孔盖铜 |

...
已完成 **20.0 × 29.0 mm、四层、标称 0.8 mm** 的 PCB 布局布线。使用 ESP32-C6 QFN40、4 MB 外置 Flash、横向 RFANT5220110A0T 陶瓷天线和 USB-C 原生 USB Serial/JTAG。最近布局将板宽缩至 20 mm，沿用右侧天线和左侧 Codex 图标布局；独立 QA 仍有既有失败条件，见下方最新检查记录。

2026-09-09 最新局部调整：**L2（2 nH、0402）移至 B.Cu**，从 (102.10, 113.00) mm 移到 (111.55, 112.65) mm，腾出正面左排针旁的原占位。其余 63 个器件、全部信号线、板框、禁布区和两个 logo 均保持输入状态；C11/C19/C20 保持正面。只调整 +3V3 / V3A 的局部连接，移除原输入过孔，新增一颗双面盖油的输出过孔，总数仍为 125。到 U1 两个供电脚的路径约 3.7–3.9 mm（含标称 0.8 mm 过孔贯穿长度）。填铜后 **DRC 0、未连接 0、一致性 0；QA 46/52**，较本次实际输入的 45/51 新增一项通过检查，原有六项失败不变。BOM 器件信息不变，装配坐标和正反面预览已同步；射频与供电表现尚未实测。最新范围、备份和验证见 [L2 迁移记录](output/l2-back-migration.json)，后文各轮数字保留为历史记录。
...
**F1 已按用户要求删除**：原理图和 PCB 均移除 1206 自恢复保险丝，USB 的 `VBUS_USB` 直接连接 D5 阳极，取消 `VBUS_FUSED` 网络。移除绕到板尾 F1 的走线，保留 D5 防反灌；其余 64 个器件的位置、安装面和封装不变。BOM、装配坐标及预览已同步，默认装配由 59 个减为 58 个。删除后的 ERC、DRC、未连接和原理图一致性均为 0；QA 仍为 45/51，六项既有失败不变。删除 F1 时的检查结果为历史记录，最新结果见上方 L2 迁移记录。
...
外围焊盘内的过孔已改为焊盘外短线扇出，并检查正反面的阻焊开窗；孔边间距至少 0.10 mm。当前仅允许 U1 中心接地焊盘的 9 个填孔盖铜过孔，实际清单为 7 个；SOIC-8 U3 无中心焊盘。为腾出局部扇出空间，调整了 C7、C11、C27、R2、R3、R7、R16，其中 R16 改到正面。
...
板上共 64 个器件位，默认装配 58 个。默认不装 D1、L3、C17、C18、C27、C28。3D 图为布局示意，部分标准库封装缺少 3D 模型；器件是否装配以 BOM 和二维装配图为准。

## 接口与下载

**从正面看**，USB 朝下、天线朝上时，J2 在左、J3 在右，两列 1 脚均靠近天线；从背面观察时左右对调，编号不倒序。排针塑料座和长针在背面，正面为焊接端。针距为 **2.00 mm**，两列中心距为 **17.50 mm**；这些尺寸不能按 2.54 mm 面包板间距处理。
...
*5V 引脚实际电压等于 USB 电压扣除 D5 压降，不是精密 5.00 V。USB 电源直接经 D5 给稳压器和 5V 引脚供电，板上不再设置自恢复保险丝；RGB 灯和电平转换器接 USB 原始电源。3V3/5V 引脚按电源输出使用，不要在连接 USB 时再向这些引脚注入外部电源。所有 GPIO 为 3.3 V 逻辑。
